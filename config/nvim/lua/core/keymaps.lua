...
map({ "i", "v", "n" }, "<C-a>", "<ESC> ggVG<CR>", { desc = "礪  select all" })
...
-- Resize with arrows
map("n", "<A-Up>", "<cmd> resize -2<CR>", { desc = "ﭕ   decrease height" })
map("n", "<A-Down>", "<cmd> resize +2<CR>", { desc = "ﭕ   increase height" })
map("n", "<A-Left>", "<cmd> vertical resize -2<CR>", { desc = "ﭕ   decrease width" })
map("n", "<A-Right>", "<cmd> vertical resize +2<CR>", { desc = "ﭕ   increase width" })
...
map("n", "<leader>us", "<cmd> e $MYVIMRC | :cd %:p:h <CR>", { desc = "  settings" })
map("n", "<leader>w", "<cmd> w! <CR>", { desc = "﬚   save file" })
map("n", "<leader>q", "<cmd>lua require('core.utils').smart_quit()<CR>", { desc = "   quit" })
map("n", "<leader>up", "<cmd> Lazy sync <CR>", { desc = "異 update plugins" })
